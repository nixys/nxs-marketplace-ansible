An Ansible role that installs Nginx and makes its initial configuration
...
| ---      | ---      | ---      |
| **nginx_version** | Nginx version. | 1.22 |
| **nginx_worker_processes** | Number of workers. | auto |
| **nginx_worker_connections** | Number of connections. | 1024 |
| **nginx_ssl_dhparam** | Adding ssl_dhparam. | no |
| **nginx_ssl_prefer_server_ciphers** | Preferred cipher. | no |
| **nginx_ssl_protocols** | List of SSL protocols. | no |
| **nginx_ssl_ciphers** | SSL ciphers. | no |
| **nginx_site_default_type** | Default config (default or error404).| default |
| **nginx_site_default_ssl** | SSL for default config. | no |
| **nginx_deploy_method** | Deployment method (host or docker). | host |
| **nginx_docker_subnet** | Docker container subnet. | 172.18.1.0/24 |
| **nginx_docker_ip** | Docker container ip. | 172.18.1.2 |
| **ansible_major_version** | Ansible major version. | 2 |
| **ansible_minor_version** | Ansible minor version. | 13 |
